...
REQUIRED_CONFIG.forEach((key) => {
  console.log("key", key);
  console.log("process", process.env.MONGO_DB_NAME, process.env[key]);
  if (!process.env[key]) {
...
let CONNECTION_URI;
console.log("TUNNEL_ENVIRONMENT", TUNNEL_ENVIRONMENT);
if (TUNNEL_ENVIRONMENT) {
  CONNECTION_URI = `mongodb://${SSH_LOCAL_HOST}:${SSH_LOCAL_PORT}/${MONGO_DB_NAME}`;
} else {
  ...
  // CONNECTION_URI =
  //   "mongodb+srv://simpliclarify:Meraki%401997@cluster0.ceuwydz.mongodb.net/simpliclarify-staging";
}
